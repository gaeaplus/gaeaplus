...
public class CompoundElevationModel extends AbstractElevationModel
{
	...
	public double getExactElevation(Angle latitude, Angle longitude){
            ...
            for (int i = this.elevationModels.size() - 1; i >= 0; i--) // iterate from highest resolution to lowest
            {
                ...
                double emValue = em.getExactElevation(latitude, longitude);
    ...
    public double getExactElevation(Angle latitude, Angle longitude, double targetResolution){
            ...
            for (int i = this.elevationModels.size() - 1; i >= 0; i--) // iterate from highest resolution to lowest
            {
                ...
                double emValue = em.getExactElevation(latitude, longitude, targetResolution);
    ...
    {
        return this.doGetElevationsImmediate(sector, latlons, targetResolution, buffer);
    }

    protected double doGetElevationsImmediate(Sector sector, List<? extends LatLon> latlons, double targetResolution,
        double[] buffer)
    {
        if (sector == null)
        {
        ...
        for (ElevationModel em : this.elevationModels)
        {
            ...
            double r = em.getElevationsImmediate(sector, latlons, targetResolution, buffer);
