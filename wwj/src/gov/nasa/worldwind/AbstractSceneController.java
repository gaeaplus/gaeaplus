...
//    protected Map<String, GroupingFilter> groupingFilters = new HashMap<String, GroupingFilter>();
    ...
    //X-START
	//Vito
	protected static final Logger logger = Logger.getLogger(AbstractSceneController.class.getName());
	private ShaderContext shaderContext = null;
	//X-END
    ...
    public void reinitialize()
    {
        ...
        //X-START
		//Vito
        //dispose layers and shader context 
		if(dc != null){
...
//        dc.setGroupingFilters(this.groupingFilters);
        ...
        //X-START
		if(this.shaderContext == null){
			this.shaderContext = new ShaderContextImpl();
		}
		dc.setShaderContext(shaderContext);
		//X-END
    ...
    protected void preRender(DrawContext dc)
    {
        try
        {
            ...
            if (dc.getLayers() != null)
            {
                ...
                //X-START
                //Vito
                //flush opengl (per-frame) task service
                WorldWind.getGLTaskService().flush(dc);
                //X-END
            }
    ...
    protected void drawLayers(DrawContext dc, LayerRenderAttributes.RenderType renderTypeFilter){
        try
        {
            ...
            if (dc.getLayers() != null)
            {
                for (Layer layer : dc.getLayers())
                {
                    //render only layers with specific render type
                    if(renderTypeFilter != null 
                            && renderTypeFilter != layer.getRenderAttributes().getRenderType()){
                        continue;
                    }
        ...
        {
            Logging.logger().log(Level.SEVERE, Logging.getMessage("BasicSceneController.ExceptionDuringLayerRendering"), e);
        }
    }
