...
public class BasicView extends WWObjectImpl implements View
{
    ...
    //X-START
	//alternative modelview matrix used only for OpenGL render calls
	private boolean useModelviewAlt = false;
	private Matrix modelviewAlt = Matrix.IDENTITY;
    private Matrix modelviewAltInv = Matrix.IDENTITY;

	//reference center stack
	private Vec4 referenceCenter = Vec4.ZERO;
	protected ArrayList<Vec4> refCenterStack = new ArrayList<Vec4>();

    //event signalling initialization is done
    public static final String DRAWCONTEXT_APPLIED = "gov.nasa.worldwind.view.orbit.BasicView.DrawContextApplied";
	//X-END
    ...
    {
		//X-START
		//set reference center to zero vector
		Vec4 initRefCenter = new Vec4(0.0d, 0.0d, 0.0d);
		this.refCenterStack.add(initRefCenter);
		//X-END
    }

	//X-START
	/**
     * Use alternative modelview matrix.
	 * Alternative modelview matrix is used when you don't want to change 
	...
	/**
     * Set alternative modelview matrix.
	 * Alternative modelview matrix is used when you don't want to change 
	 * rendered data LOD or visibility, because of change in eye position.
	 * 
	...
	public void setModelviewMatrixAlt(Matrix matrix, boolean effectFrustum){
		this.modelviewAlt = matrix;
		this.modelviewAltInv = matrix.getInverse();
		if(effectFrustum){
			Matrix modelviewTranspose = matrix.getTranspose();
	...
	public void setModelviewMatrix(Matrix matrix)
    {
		...
		this.modelviewInv = matrix.getInverse();
    ...
    public void apply(DrawContext dc)
    {
        ...
        //X-PATCH
        //notify listeners that initialization is done
        this.firePropertyChange(DRAWCONTEXT_APPLIED, null, this.globe);
        //X-END
    }
    ...
    public Matrix pushReferenceCenter(DrawContext dc, Vec4 referenceCenter)
    {
		...
		//X-START
		//alternative modelview
		if(this.useModelviewAlt){
			if (modelviewAlt != null){
            	matrix = modelviewAlt.multiply(Matrix.fromTranslation(referenceCenter));
			}	
		}
		else{
        	if (modelview != null){
        ...
        try
        {
            ...
            if (matrix != null)
            {
				...
				//X-START
				//push reference center parameter (shaders)
				this.refCenterStack.add(referenceCenter);
				...
				if(shader != null){
					...
					//GL_LIGHT7 diffuse collor attribute is abused for reference center vector
					gl.glLightfv(GL2.GL_LIGHT7, GL2.GL_DIFFUSE, referenceCenter.toFloatArray(rc, 0, 4), 0);
				}
				//X-END
            }
        }
    ...
    public void popReferenceCenter(DrawContext dc)
    {
        ...
        try
        {
			...
			//X-START
			//pop reference center parameter (shaders)
			Shader shader = dc.getShaderContext().getCurrentShader();
			int index = this.refCenterStack.size() - 1;

			if(index >= 0){
				this.referenceCenter = this.refCenterStack.get(index);
			}
			...
			if(shader != null && index >= 0){
				...
				Matrix matrix = null;
				if (modelview != null)
					matrix = modelview.multiply(Matrix.fromTranslation(this.referenceCenter));

				if(matrix != null){
					float[] rc = new float[4];
					//GL_LIGHT7 diffuse collor attribute is abused for reference center vector
					gl.glLightfv(GL2.GL_LIGHT7, GL2.GL_DIFFUSE, referenceCenter.toFloatArray(rc, 0, 4), 0);
					//remove vector from stack
					//keep zero vector (index == 0) in stack
					if(index > 0)
						this.refCenterStack.remove(index);
				}
			}
			//X-END
        }
