...
public class RenderableLayer extends AbstractLayer
{
    ...
    /**
     * Returns true iff the layer contains the given renderable (that is, a renderable for which equals() returns true).
     * @param renderable
     * @return 
     */
    public boolean containsRenderable(Renderable renderable)
    {
        if (this.renderablesOverride != null)
        {
            Iterator<Renderable> iter = getRenderables().iterator();
            while (iter.hasNext())
            {
                if (iter.next().equals(renderable))
                    return true;
            }
            return false;
        } else return
            this.renderables.contains(renderable);
    }
    //X-END
