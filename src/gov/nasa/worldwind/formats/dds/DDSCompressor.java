...
public class DDSCompressor
{
    ...
    //X-PATCH Marjan
    //added transp. filter argument to compressImage(java.awt.image.BufferedImage image)
    /**
     ...
     *                                  dimensions.
     */
    //the original method changed so that it uses null transp. filter
    public static java.nio.ByteBuffer compressImage(java.awt.image.BufferedImage image)
    {
        return compressImage(image, (ITransparencyFilter)null);
    }
    
    //new method added that accepts transp. filter argument
    public static java.nio.ByteBuffer compressImage(java.awt.image.BufferedImage image, ITransparencyFilter transparencyFilter)
    //X-END
    {
        if (image == null)
        {
        ...
        //X-PATCH Marjan
        //applying the transp. filter to attributes
    ...
    protected DXTCompressor getDXTCompressor(java.awt.image.BufferedImage image, DXTCompressionAttributes attributes)
    {
        ...
        //X-PATCH Marjan
        //only allow DXT1 (=no alpha channel) if no transparency filter is set
        else if (!image.getColorModel().hasAlpha() && null == attributes.getTransparencyFilter())
        //X-ORIG
